...
class SettingsScreenViewModelTest {
    ...
    @Test
    fun testFadePlaybackChange() = runTest {
        backgroundScope.launch( UnconfinedTestDispatcher() ) { subject.uiState.collect() }
    ...
    fun testFadePlaybackDurationChange() = runTest {
        ...
        ( 1 until 10 ).forEach {
            subject.setFadePlaybackDuration( it.toFloat() )
            assertEquals(
                SettingsScreenUiState.Success(
                    userData = emptyUserData.copy(
                        fadePlaybackDuration = it.toFloat()
                    )
                ),
                subject.uiState.value
            )
    ...
    @Test
    fun testRequireAudioFocusChange() = runTest {
        backgroundScope.launch( UnconfinedTestDispatcher() ) { subject.uiState.collect() }
        ...
        setOf( true, false ).forEach {
            subject.setRequireAudioFocus( it )
            assertEquals(
                SettingsScreenUiState.Success(
                    userData = emptyUserData.copy(
    ...
    @Test
    fun testIgnoreAudioFocusLossChange() = runTest {
        backgroundScope.launch( UnconfinedTestDispatcher() ) { subject.uiState.collect() }
        ...
        setOf( true, false ).forEach {
            subject.setIgnoreAudioFocusLoss( it )
            assertEquals(
                SettingsScreenUiState.Success(
                    userData = emptyUserData.copy(
